resource "aws_instance" "stagehaproxy" {
  connection {
  ...
  tags {
    Name          = "stagehaproxy_${random_id.instance_id.hex}"
    X-Dept        = "${var.tag_dept}"
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo hostnamectl set-hostname stage-haproxy-1",
    ]
  }
  ...
  provisioner "habitat" {
    ...
    service {
      ...
      strategy = "at-once"
      group = "stage"
      binds = ["backend:national-parks.stage"]
  ...
  provisioner "file" {
    ...
    content     = "${data.template_file.enable_automate_eas_linux_stagehaproxy.rendered}"
  }
...
data "template_file" "enable_automate_eas_linux_stagehaproxy" {
...
output "haproxy_stage" {
  value = "http://${element(concat(aws_instance.stagehaproxy.*.public_ip, list("")), 0)}"
}
