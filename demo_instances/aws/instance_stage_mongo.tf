resource "aws_instance" "stagemongo" {
  connection {
  ...
  tags {
    Name          = "stagemongo_${random_id.instance_id.hex}"
    X-Dept        = "${var.tag_dept}"
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo hostnamectl set-hostname stage-mongo-1",
    ]
  }
  ...
  provisioner "chef" {
    ...
    node_name = "stage-mongo-1"
  }

  provisioner "file" {
    destination = "/tmp/enable_eas.sh"
    content     = "${data.template_file.enable_automate_eas_linux_stagemongo.rendered}"
  }
...
data "template_file" "enable_automate_eas_linux_stagemongo" {
