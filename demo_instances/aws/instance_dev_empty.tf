resource "aws_instance" "dev_empty" {
  connection {
  ...
  tags {
    Name          = "dev_empty_${random_id.instance_id.hex}"
    X-Dept        = "${var.tag_dept}"
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo hostnamectl set-hostname dev-empty",
    ]
  }
...
output "ssh_empty_public_ip_dev" {
  value = "${aws_instance.dev_empty.public_ip}"
}
