resource "aws_instance" "stage_empty" {
  connection {
  ...
  tags {
    Name          = "stage_empty_${random_id.instance_id.hex}"
    X-Dept        = "${var.tag_dept}"
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo hostnamectl set-hostname stage-empty",
    ]
  }
...
output "ssh_empty_public_ip_stage" {
  value = "${aws_instance.stage_empty.public_ip}"
}
